...
import {  getOrdersByIdByAdmin,  resetOrder, updateCloseStatusOfOrder } from '../Reducers/Actions/OrderAction'
...
const OrderDetailScreenAdmin = () => {
    ...
    const CompleteDeliveryFunction = async () => {
        setIsloading(true)
        await dispatch(updateCloseStatusOfOrder(orderId))
        await dispatch(getOrdersByIdByAdmin(orderId))
        setIsloading(false)
    }
         ...
         <View style={tw('flex-1')}>
            ...
            {order && (
                <ScrollView showsVerticalScrollIndicator={false} style={tw('flex-1 px-2 pt-4 pb-4')}>
                   ...
                   {order.status == "OPEN" &&  <Button onPress={CompleteDeliveryFunction}  titleStyle={tw('text-zinc-700  text-lg')} title="Complete dellivery" buttonStyle={tw('w-full font-bold text-black text-lg bg-amber-300 rounded-lg py-4 my-2 mb-8')}></Button>}                
...
export default OrderDetailScreenAdmin
